...
import { CustomerManagementSection, VoucherManagementSection } from '@pages';
...
export const VoucherManagementModule = (props: IProps) => {
  ...
  const handleViewDetails = (id: number | string) => {
    ...
    navigate(PATH_CAMPAIGN + PATH_VOUCHER + `/details/${id}`);
  };

  const onEditProfile = (id: number | string) => {
    navigate(PATH_CAMPAIGN + PATH_VOUCHER + `/edit/${id}`);
  };

  const onCreate = () => {
    navigate(PATH_CREATE_VOUCHER);
  };
