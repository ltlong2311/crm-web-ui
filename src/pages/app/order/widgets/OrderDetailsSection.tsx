...
import { generateRandomId, genUID, LogApp } from '@utils';
...
export const OrderDetailsSection = (props: IProps) => {
  const {
    products,
    data,
    errors,
          ...
          <DropdownButton
            ...
            dropdown={
              <OptionDropdown>
                ...
                <div className="option-item">Xuất hoá đơn</div>
                <div className="option-item dangerous">Xóa đơn hàng</div>
              </OptionDropdown>
            }
          />
        </div>
          ...
          <form onSubmit={onSubmit}>
            {data?.status && (
              <div className="order-status">
                <p className="order-label">Trạng thái</p>
            ...
            <div className="more-info">
              <p className="order-label">Ngày đặt hàng</p>
              <ShareDateSelect
                containerClassName="input-contain"
                inputClassName={'time-select from-time-input mb-4'}
                minDate={moment().startOf('day')}
                format={dateFormat}
                key={`createdDate-${data?.deliveryDate}`}
                defaultValue={data?.deliveryDate && moment(Number(data.deliveryDate))}
                onChange={(date, dateString) => {
                  //
                }}
                noBorderStyle
              />
              <div className="order-label">Ngày giao dự kiến</div>
              <ShareDateSelect
                containerClassName="input-contain"
                ...
                showTime
                format={dateFormat}
                key={`deliveryDate-${data?.deliveryDate}`}
                defaultValue={data?.deliveryDate && moment(Number(data.deliveryDate))}
                onChange={(date, dateString) => {
              ...
              {diffDeliverDays && (
                <StatusTag
                  className="mt-3"
                  label={`Trong vòng ${diffDeliverDays} ngày`}
                  backgroundColor={APP_COLORS.yellow800}
                />
              )}
            </div>
            <div className="more-info">
