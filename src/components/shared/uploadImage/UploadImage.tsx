...
export const ShareUploadImage = (props: IProps) => {
  ...
  return typeUpload === 'button' ? (
    <StyledUploadImage $previewImageStyle={previewImageStyle}>
      <div className="contain-upload">
        <SharedButton
          ...
          backgroundColor={APP_COLORS.cyan700}
          text="Choose image"
          btnStyle="basic"
        />
        <div>
    ...
    <StyledInfoItem className="upload__image" $previewImageStyle={previewImageStyle} $typeUpload={typeUpload}>
      {typeUpload === 'change' ? (
        <>
          ...
          <SharedButton
            ...
            className="upload-btn"
            backgroundColor={APP_COLORS.cyan700}
            text={btnUploadText || 'Thay đổi ảnh'}
            btnStyle="basic"
          />
        </>
      ) : (
...
const StyledInfoItem = styled.div<{
  ...
  ${({$typeUpload}) => $typeUpload==='change' && css`
    height: fit-content;
  `}
`;
