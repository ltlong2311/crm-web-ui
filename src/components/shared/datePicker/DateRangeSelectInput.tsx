...
export const ShareRangeDateSelect = memo((props: IPropsDatePicker) => {
  //page props
  const {
    type,
    format = 'DD-MM-YYYY',
  ...
  return (
    <StyledSelectDateContainer className={containerClassName} $noBorderStyle={noBorderStyle}>
      ...
      <StyledRangeDatePicker
        ...
        popupClassName={
          hiddenTimePanel
            ? 'app-range-picker-hidden-time ignore-onClickOutside'
            : 'app-range-picker-popup ignore-onClickOutside'
        }
        // onOk={(dates: [any, any]) => {
        //   if (dates[1]) {
        //     setPickerOpen(false);
